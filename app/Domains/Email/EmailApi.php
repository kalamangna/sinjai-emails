...
class EmailApi extends BaseController
{
    ...
    public function api_pppk_list()
    {
        try {
            $statusPppk = $this->statusAsnModel->where('nama_status_asn', 'PPPK')->asArray()->first();

            if (!$statusPppk) {
                return $this->response->setJSON(['success' => false, 'message' => 'Status PPPK belum dikonfigurasi di sistem.']);
            }
            ...
            $emails = $emailBuilder->orderBy('emails.name', 'ASC')->findAll();
    ...
    public function api_pppk_pw_list()
    {
        try {
            $statusPppkPw = $this->statusAsnModel->where('nama_status_asn', 'PPPK PARUH WAKTU')->asArray()->first();

            if (!$statusPppkPw) {
                return $this->response->setJSON(['success' => false, 'message' => 'Status PPPK PARUH WAKTU belum dikonfigurasi di sistem.']);
            }
