...
class EmailService
{
    ...
    public function normalizeJabatanName(?string $jabatan, ?string $unitKerjaName = null, bool $isPimpinan = false): ?string
    {
        ...
        // Bersihkan embel-embel nama instansi di ujung nama Jabatan (Struktural, JF, & Staf Pelaksana)
        if (preg_match('/^(KEPALA BIDANG|KEPALA SUB BAGIAN|KEPALA SEKSI|KEPALA SUB BIDANG|PENGADMINISTRASI|PENGELOLA|OPERATOR|PRANATA|BENDAHARA|PENGEMUDI|PRAMU|PETUGAS|STAF|TEKNISI|FASILITATOR|PENYUSUN|PEMERIKSA|ANALIS|GURU|DOKTER|PERAWAT|BIDAN|APOTEKER|EPIDEMIOLOG|SANITARIAN|NUTRISIONIS|ARSIPARIS|PUSTAKAWAN|AUDITOR|PERENCANA|PENYULUH|INSTRUKTUR|STATISTISI|PENELITI|PENGUJI|POLISI PAMONG PRAJA)\b/i', $jab)) {
            $jab = preg_replace('/\s+(SEKRETARIAT|PADA|DI|KEC\.|KECAMATAN|DINAS|BADAN|INSPEKTORAT|SATPOL PP|SATUAN POLISI|UPTD RSUD|RSUD|UPTD PUSKESMAS|PUSKESMAS|UPTD|KELURAHAN|KEL\.|BAGIAN|IFK|GFK|SKB)\s*.*$/i', '', $jab);
        }
