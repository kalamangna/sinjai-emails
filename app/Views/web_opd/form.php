        ...
        <div class="p-8">
            <form action="<?= site_url('web_opd/update/' . $website['id']) ?>" method="POST" id="websiteForm" class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t border-slate-100">
                    <a href="<?= site_url('web_opd') ?>" class="w-full sm:w-auto px-6 py-2 bg-white border border-slate-200 text-slate-700 font-bold rounded-lg hover:bg-slate-50 transition-all text-xs uppercase tracking-widest no-underline flex items-center justify-center shadow-sm">
