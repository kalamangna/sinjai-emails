            ...
            <form id="auditFilterForm" method="GET" action="<?= site_url('audit-trail') ?>" class="hidden md:grid grid-cols-1 md:grid-cols-12 gap-y-4 gap-x-4 items-end">
                ...
                <div class="md:col-span-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="<?= esc($search) ?>" class="block w-full pl-9 pr-3 py-2 bg-white border <?= !empty($search) ? 'border-slate-800 ring-1 ring-slate-800' : 'border-slate-200' ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm text-slate-800 placeholder-slate-400 font-medium transition-all" placeholder="Nama atau username...">
                    </div>
                </div>
