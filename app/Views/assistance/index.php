        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50">
            <form method="GET" action="" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div class="md:col-span-4">
                    ...
                    <select name="category" class="block w-full px-3 py-2 bg-white border <?= !empty($filterCategory) ? 'border-slate-800 ring-1 ring-slate-800' : 'border-slate-200' ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm appearance-none cursor-pointer transition-all">
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="month" class="block w-full px-3 py-2 bg-white border <?= !empty($filterMonth) ? 'border-slate-800 ring-1 ring-slate-800' : 'border-slate-200' ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm appearance-none cursor-pointer transition-all">
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="year" class="block w-full px-3 py-2 bg-white border <?= !empty($filterYear) ? 'border-slate-800 ring-1 ring-slate-800' : 'border-slate-200' ?> rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm appearance-none cursor-pointer transition-all">
                ...
                <div class="md:col-span-2 flex gap-2">
                    ...
                    <a href="<?= site_url('assistance') ?>" class="btn btn-outline" title="Reset">
        ...
        <?php if (!empty($activities)): ?>
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="text-[10px] font-bold text-slate-700 uppercase tracking-widest">
                    ...
                    $end = $start + count($activities) - 1;
                    ?>
                    Menampilkan <span class="text-slate-800"><?= $start ?> - <?= $end ?></span> dari <span class="text-slate-800"><?= number_format($totalActivities ?? 0, 0, ',', '.') ?></span> log
                </div>
