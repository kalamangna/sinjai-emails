...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 uppercase tracking-tight">Email</h1>
            <?php if (!empty($last_sync_time)): ?>
                <p class="text-[10px] font-bold text-gray-700 uppercase tracking-widest mt-1">
                    <i class="fas fa-history mr-1"></i> Terakhir Sync:
                    <span class="text-gray-800"><?= formatTanggalWaktu($last_sync_time) ?></span>
                </p>
            <?php endif; ?>
        </div>
        ...
        <div class="flex items-center gap-2 w-full lg:w-auto">
            <?php if (session()->get('role') === 'super_admin'): ?>
                <button onclick="confirmSyncCpanel(this)" data-href="<?= site_url('email/sync') ?>" class="flex-1 lg:flex-none inline-flex items-center justify-center px-4 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 border-transparent rounded-lg font-bold text-xs uppercase tracking-widest transition-all shadow-sm group" id="syncCpanelBtn">
                    <i class="fas fa-sync-alt mr-2 group-hover:rotate-180 transition-transform duration-500"></i>
                    <span>Sync cPanel</span>
                </button>
                <a href="<?= site_url('batch') ?>" class="flex-1 lg:flex-none inline-flex items-center justify-center px-4 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 border-transparent rounded-lg font-bold text-xs uppercase tracking-widest transition-all shadow-sm no-underline">
                    <i class="fas fa-layer-group mr-2"></i> Batch
                </a>
                <a href="<?= site_url('email/create') ?>" class="flex-1 lg:flex-none inline-flex items-center justify-center px-4 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg font-bold text-xs uppercase tracking-widest transition-all shadow-sm no-underline">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
            ...
            <h3 class="text-2xl font-bold text-gray-800 mt-1"><?= number_format($total_emails ?? 0, 0, ',', '.') ?></h3>
        </div>
        <div class="bg-white border border-gray-200 border-l-4 border-l-emerald-700 rounded-lg shadow-sm p-6">
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Email Aktif</p>
            <h3 class="text-2xl font-bold text-gray-800 mt-1"><?= number_format($active_count ?? 0, 0, ',', '.') ?></h3>
        </div>
        <div class="bg-white border border-gray-200 border-l-4 border-l-emerald-700 rounded-lg shadow-sm p-6">
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">TTE Aktif</p>
            <h3 class="text-2xl font-bold text-gray-800 mt-1"><?= number_format($active_bsre_count ?? 0, 0, ',', '.') ?></h3>
        </div>
    </div>
        ...
        <div class="p-6 border-b border-gray-100 bg-gray-50">
            <form method="GET" action="<?= site_url('email') ?>" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                ...
                <div class="md:col-span-2 lg:col-span-2 flex gap-2">
                    ...
                    <a href="<?= site_url('email') ?>" class="inline-flex items-center justify-center px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 border-transparent rounded-lg transition-all shadow-sm" title="Reset">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <?php if (!empty($email['parent_unit_kerja_name'])): ?>
                                            <span class="text-[10px] font-bold text-gray-700 uppercase leading-none"><?= esc($email['parent_unit_kerja_name']) ?></span>
                                            <span class="text-xs font-bold text-gray-800 uppercase tracking-tight mt-1"><?= esc($email['unit_kerja_name']) ?></span>
                                        <?php else: ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="<?= site_url('email/detail/' . $email['user']) ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 border-transparent transition-all shadow-sm" title="Detail">
        ...
        <?php if (isset($pager)): ?>
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="text-[10px] font-bold text-gray-700 uppercase tracking-widest">
                    Menampilkan <span class="text-gray-800"><?= count($emails) ?></span> dari <span class="text-gray-800"><?= number_format($total_emails ?? 0, 0, ',', '.') ?></span> akun
                </div>
