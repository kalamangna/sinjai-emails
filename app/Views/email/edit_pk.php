...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-slate-800 uppercase tracking-tight">Edit Perjanjian Kerja (PK)</h1>
    </div>
            ...
            <div class="p-8 space-y-10">
                ...
                <div class="space-y-6">
                    <div class="flex justify-between items-center border-b border-slate-100 pb-1">
                        <h4 class="text-[10px] font-bold text-slate-700 uppercase tracking-widest">Informasi Pegawai</h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Nama Lengkap</label>
                            <input type="text" value="<?= esc($email['name']) ?>" class="block w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium text-slate-500 uppercase cursor-not-allowed" readonly>
                        </div>
                        <div>
                            <label for="nomor" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Nomor PK</label>
                            <input type="text" name="nomor" id="nomor" value="<?= esc($pk_data['nomor'] ?? '') ?>" class="block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 text-sm font-medium text-slate-800 transition-all font-mono" placeholder="Contoh: 001/PK/2026" required>
                        </div>
                        <div></div>
                        <div>
                            <label for="tanggal_kontrak_awal" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Kontrak Mulai</label>
                            <input type="date" name="tanggal_kontrak_awal" id="tanggal_kontrak_awal" value="<?= esc($pk_data['tanggal_kontrak_awal'] ?? '') ?>" class="block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 text-sm font-medium text-slate-800 transition-all" required>
                        </div>
                        <div>
                            <label for="tanggal_kontrak_akhir" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Kontrak Selesai</label>
                ...
                <!-- Bagian: Penghasilan -->
                <div class="space-y-6">
                    <h4 class="text-[10px] font-bold text-slate-700 uppercase tracking-widest border-b border-slate-100 pb-1">Penghasilan / Gaji</h4>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="gaji_nominal" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Nominal Gaji (Rp)</label>
                            <div class="relative">
                                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-500 text-sm">Rp</span>
                                <input type="text" name="gaji_nominal" id="gaji_nominal" value="<?= number_format($pk_data['gaji_nominal'] ?? 0, 0, ',', '.') ?>" class="block w-full pl-10 pr-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 text-sm font-bold text-slate-800 transition-all" required>
                            </div>
                        </div>
                        <div class="md:col-span-2">
                            <label for="gaji_terbilang" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Terbilang (Rupiah)</label>
