...
<div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row items-center gap-6">
                ...
                <div class="text-center md:text-left space-y-2">
                    <div class="flex flex-col md:flex-row md:items-center gap-2">
                        <p class="text-xl sm:text-2xl font-bold text-slate-800 break-all"><?= esc($email['email']) ?></p>
                    ...
                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-4">
                        ...
                        <?php if (!empty($email['pensiun_at'])): ?>
                            <span class="px-2.5 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-red-100 text-red-700 border border-red-200">
                                <i class="fas fa-user-slash mr-1"></i> PENSIUN / PINDAH / KELUAR
                            </span>
                        <?php endif; ?>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap mt-0.5">
                                        ...
                                        <?php if ($bupInfo && ($bupInfo['is_approaching'] || $bupInfo['is_pensiun'])): ?>
                                            <?php
                                            $bupBadgeClass = $bupInfo['is_pensiun']
                                                ? 'bg-rose-50 text-rose-700 border-rose-200'
                                                : 'bg-amber-50 text-amber-700 border-amber-200';
                                            ?>
                                            <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase border <?= $bupBadgeClass ?>" title="<?= esc($bupInfo['sisa_waktu_label']) ?>">
    ...
    <?php if (in_array(session()->get('role'), ['super_admin', 'admin'])): ?>
        <div class="mt-6 flex flex-wrap items-center justify-end gap-3">
            <?php if (empty($email['pensiun_at'])): ?>
                <form action="<?= site_url('email/mark_pensiun/' . $email['user']) ?>" method="POST" class="inline" onsubmit="return confirm('Tangguhkan akun ini (Pensiun / Pindah / Keluar)? Akses login email akan ditangguhkan dan data pegawai akan dilepas.');">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-outline text-amber-600 border-amber-300 hover:bg-amber-100 hover:border-amber-400" title="Tandai Pensiun / Pindah / Keluar">
                        <i class="fas fa-user-slash mr-1.5"></i> Pensiun / Pindah / Keluar
                    </button>
                </form>
            <?php endif; ?>
            <?php if (session()->get('role') === 'super_admin'): ?>
                <form action="<?= site_url('email/delete/' . $email['id']) ?>" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini?');">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-solid-danger" title="Hapus Akun">
