    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php if (!empty($eselons)): ?>
            <?php foreach ($eselons as $eselon): ?>
                <a href="<?= site_url('email/eselon_detail/' . $eselon['id']) ?>" class="group bg-white border border-slate-200 rounded-xl p-6 hover:border-slate-800 hover:shadow-md transition-all no-underline flex flex-col">
