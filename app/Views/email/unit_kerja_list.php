...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="unitGrid">
        <?php foreach ($unit_kerja as $unit): ?>
            <a href="<?= site_url('email/unit_kerja/' . $unit['id']) ?>" class="group bg-white border border-slate-200 rounded-lg p-6 hover:border-slate-800 hover:shadow-md transition-all no-underline unit-card flex flex-col" data-name="<?= esc(strtoupper($unit['nama_unit_kerja'])) ?>">
