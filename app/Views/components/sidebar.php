<aside id="sidebar" class="fixed top-0 left-0 z-50 w-64 h-screen transition-transform bg-slate-800 border-r border-slate-700 flex flex-col lg:translate-x-0 -translate-x-full">
    ...
    <nav class="flex-grow py-6 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <!-- Verifikasi PDF -->
        <a href="<?= site_url('verifikasi-pdf') ?>" target="_blank" rel="noopener noreferrer" data-sidebar-clear class="flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-all <?= $isActive('verifikasi-pdf') ? 'bg-slate-700 text-white shadow-lg shadow-slate-900/20' : 'text-slate-100 hover:bg-slate-700/80 hover:text-white' ?>">
            <div class="w-5 h-5 flex items-center justify-center mr-3 shrink-0">
                <i class="fas fa-file-signature <?= $isActive('verifikasi-pdf') ? 'text-white' : 'text-slate-300' ?>"></i>
            </div>
            Verifikasi PDF
        </a>
