...
<div class="space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-lg shadow-sm max-w-2xl mx-auto">
        <div class="bg-slate-50 px-6 py-4 border-b border-slate-200">
            <h3 class="text-xs font-bold text-slate-800 uppercase tracking-tight">Informasi User Baru</h3>
        </div>
        <form action="<?= site_url('auth/users/store') ?>" method="POST" class="p-6 space-y-4">
            <?= csrf_field() ?>

            <div>
                <label for="username" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Username</label>
                <input type="text" id="username" name="username" value="<?= old('username') ?>" required class="block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm font-medium text-slate-800 transition-all" placeholder="Masukkan username...">
            </div>

            <div>
                <label for="password" class="block text-sm font-medium text-slate-700 mb-1 uppercase tracking-tight">Password</label>
                <input type="password" id="password" name="password" required class="block w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-700 focus:border-slate-700 text-sm font-medium text-slate-800 transition-all" placeholder="Masukkan password...">
                <p class="mt-1 text-[10px] text-slate-500 uppercase font-bold tracking-tight">Minimal 6 karakter.</p>
            </div>
